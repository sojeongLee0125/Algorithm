...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // 다음 L개의 줄에는 뱀의 방향 변환 정보가 주어지는데 정수 X와 문자 C 이다.
        // 게임 시작 시간으로부터 X초가 끝난 뒤에 왼쪽(C가 'L') 또는 오른쪽(C가 'D')로 
        // 90도 방향을 회전시킨다는 뜻이다.
        ...
        // 1. 뱀은 0,0 에서 오른쪽으로 이동하면서 시작.
        // 2. 현재 위치에서 다음칸을 탐색한다.
        // 2-1. 다음 칸에 사과가 있을 경우 해당 칸을 차지한다.
        // 2-2. 다음 칸에 사과가 없을 경우 해당 칸은 차지하고 맨 뒷 칸은 지운다.
        // 3. 뱀이 이동할 때 한 칸씩 나갈수록 맨 뒷칸은 지워줘야 한다.
        // 4. 다음 칸이 자기 몸이거나 벽이면 게임은 끝난다.

        // 1) deque에 뱀의 현재 위치를 저장한다.
        // 2) 다음에 사과가 있으면 그 위치를 앞에 저장한다.
        // 3) 다음에 사과가 없으면 그 위치는 앞에 저장하고 맨 뒷 좌표는 뺀다.
        ...
        while (true) {
            ...
            if (ny < 0 || nx < 0 || ny >= N || nx >= N) break; // 벽
            if (chk[ny][nx] == 1) break; // 자기 몸
